...
使用者已放行六幣回測。**
**下一步：重跑六幣 ict-backtest。判準照舊 ≥20 筆且 E 明顯>0（零滑價視為上界）；
<20 筆的結論是「派網 35 天資料不足以檢定」→ 幣安長歷史回測（使用者已表態這是
下一步；涵蓋 2024 多頭/2025 盤整/2026 熊市、分段報告）或 paper 前瞻 2–3 個月，
不是放寬參數。**
...
- 尚未驗：VM 上實際打幣安 API（GCP 對外應通；若被地區限制改用 `data.binance.com`
  或 `api.binance.us` 需再調 base_url）。六幣派網回測結果仍是決定是否真走幣安的關卡。
