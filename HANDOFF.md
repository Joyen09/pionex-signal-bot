...
**網格壓測最終結論（2026-07-17，兩輪壓測＋高原/跨幣驗證完成）：**
- 儀器修正：回測 ATR/ADX 改用 indicator_interval（60M）聚合、只用已收盤桶
  （首輪在 15M 上算，區間偏窄、重開灌水，對網格系統性偏悲觀——已修）。
- 實盤儀表（15.5 天）：真實現金流 +2.18（帳面 +2.59 差額=手續費），尚無重開
  倒貨事件。年化外插無參考價值（樣本太短、且正處現行參數最有利的市況）。
- 現行參數（atr6/adx30）跨市況：2024 −5.15%／2025 −30.51%／2026 +0.78%——
  不穩健，但**「改成 atr8」的假設被否決**：BTC 上寬格在 2026 熊市反轉變差
  （寬格=重開少=漲勢優點、跌勢抱著存貨陪跌）；ETH 2025 段 atr8/adx30 = −33.41%
  與 BTC 同格 +8.08% 完全相反（跨幣不一致=路徑運氣）。
- **決定：實盤參數不動。** 跨表格一致的訊號只有：無全天候參數、ADX 過濾有用
  （保持開）、太窄（atr4）必死。真正有槓桿的改進是執行成本（自製=市價單，
  派網內建網格=限價單）——若使用者想放大網格資金再評估，不主動推。
- 監控節奏：定期 `grid-report`（~/bot）看真實現金流；重開倒貨是設計內成本
  （壓測 2026 段半年 12 次），發生時對照儀表判斷是否在預期內，不要慌改參數。
