...
我自己在 TradingView 上用 Pine Script 寫過一些策略（EMA / ADX / ATR 那類），但 TradingView 只能跳警報、不能真的下單。手動盯著警報再去交易所按買賣，一來會漏、二來人一緊張就會亂改規則。所以我想要一個東西：**不管訊號從哪來（我自己的指標、TradingView、還是手動），都走同一套風控、同一套下單流程，機械式執行。** 訊號歸訊號，執行歸執行，這樣我換策略時不必重寫下單邊輯，下單邊輯也只需要驗證一次。

## 我在意的幾個工程細節

這個專案我花最多心思的不是策略多炋，而是**怎麼讓自動下單不要出錯**，因為一旦接上真錢，一個重複下單的 bug 就是真實的虧損。幾個我刻意處理的點：

- **防止重複成交。** 我只用「已收盤」的 K 線評估訊號，而且每根 K 線只評估一次。原因是如果用「形成中」的 K 線，價格還在動，訊號會反覆跳進跳出，造成重複下單。這個坑我一開始沒想到，是測試時發現訊號亂跳才回頭修的。
- **單一部位、進出成對。** 空手才買、有倆才賣，持倆時不會一直加碼，避免越攛越平、爆掉資金。
- **多層風控。** 單筆金額上限、持倆上限、當日虧損上限、交易冷卻時間，全部在下單前檢查；任何一關不過就不下單。
...
- **自動網格**：在現價附近建網格機械式低買高賣，跌破區間自動平倆、再以新價位開新網格。
- **完整記錄與通知**：SQLite 記錄每筆交易與持倆、已實現損益；可推 Discord / Telegram / LINE 通知（啟動、成交、錯誤、每日損益摘要）。Discord 只要建個 Webhook 貼上網址就能用，設定最簡單。
...
> 順勢策略（如 `ma_cross`）通常關閉停利、留停損當安全網表現較好——它本來就靠死交叉讓獲利奔跑，硬加停利會砰太早。實際請以 `backtest-sweep` 在你自己的資料上的結果為準。
...
Webhook 對外請務必加反向代理 + HTTPS（Caddy / Nginx + Let's Encrypt），並設好 `WEBHOOK_SECRET`。`data/` 與 `logs/` 已挂載為 volume，容器重啟不會遺失交易記錄。
...
pionexbot/
  ...
  backtest.py            # 回測引擎
  store.py               # SQLite 交易與持倆記錄
  notifier.py            # 日誌 + Discord / Telegram / LINE
