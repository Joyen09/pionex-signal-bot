...
> 回測只抓公開 K 線，**不需要 API 金鑰**。回測假設以「當根收盤價」成交、含手續費、不使用槓桿、不做空，且與實盤一樣是「單一部位、進出成對、套用停損停利」，結果僅供參考，實盤會有滑點與延遲。
...
會對多組停損/停利回測並排名，最後印出「報酬最高」與「風險調整最佳（報酬/回撤）」兩種建議，並直接給你可貼進 `config.yaml` 的數值。

> 順勢策略（如 `ma_cross`）通常「關閉停利、留停損當安全網」表現較好——因為它本來就靠死亡交叉讓獲利奔跑，硬加停利會砍早。實際請以此指令在你的真實資料上的結果為準。
